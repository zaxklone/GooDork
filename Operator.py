...
class Operator:
    ...
    def setGoogleResLimit(self, limit):
    ...
    # builds a list of resutls, prevents redundant calls to servers
    def buildResults(self, links):
        # simply gets the HTML for the page, and appends it to a results list
        # I could have bs4 implemented in the Result obj,
        # but i feel its not neccessary now
        if self.ResList or len(self.ResList) == 0:
            for link in links:
                # I must remeber to add the response headers to
                # the result object
                HTML = self.getHTML(link)
                # i know its a lil redundant to have the link in
                # the Result object aswell
                res = Result.Result(link, [], "", [], HTML)
                # but I intend ot use this object as an easy way to dump
                # info to a database/XML file later
                self.ResList[link] = res  # add this to the results dictionary
                # I decided on a dictionary because its easier
                # for GooDork to work with
    ...
    def _intext(self, pattern, url):
        ...
        for string in soup(res.HTML):
            if re.search(pattern, res.HTML) is not None:
    ...
    def intext(self, pattern, url):
        ...
        try:
            ...
            # this happend when the file is not HTML!, I'm gonna fix this later,
            # so you can search SQL/XML files aswell
            return re.search(pattern, html) is not None  # there i fix!!
        return False
    ...
    """
    def _inurl(self, pattern, url):
        # the new methods, implimenting the Result object
        try:
            res = self.Res_list[url]
        except KeyError:
            return False
        hasPat = (re.search(pattern, res.URL) is not None)
        if hasPat:
    ...
    def _intitle(self, pattern, url):
        try:
            ...
            return False
        hasPat = (re.search(pattern, res.title) is not None)
        if hasPat:
    ...
    def _inanchor(self, pattern, url):
        try:
            res = self.Res_list[url]
        except KeyError:
            return False
        ...
        for anchor in (res.HTML).findAll("a"):
            ...
            if re.search(pattern, href) is not None:
                res.Summary += "*\n".join(re.match(pattern, href))
                hasPat = True
        return hasPat

    def inanchor(self, pattern, url):
        isFound = False
        html = self.ResList[url].HTML
        if html is False:
            return html
    ...
    """This has yet to be implemented
        I hope to be able to have users supply a dork and return have
        Operator return all the URLs 'related' to the urls from the dork query
        e.g
            dork .php?*wp-content*=* -related
        will return all the URLS that are related to the results returned
        from the dork
    """
    def related(self):
        # this is gonna take a lil thought to apply properly,
        # will be quite powerful!
        return
    """This has yet to be implemented
        This will apply the regex pattern to the domain of the given url string
    """
    def site(self, url, pattern):
        return
    """Search the <input> tags of a page for the supplied regex pattern
        in the vales of the supplied attribute
        e.g
         "    ininput(example.com,type,hidden) "
            will return True if the are input tags
            where the attribute type is set to hidden i.e <input type=hidden>
         "    -ininput [\w]=[\d] " will return True if there are any input tags
            where any attributes set to integer data  i.e
            <input abcdefgh123456709=1>

        url --- the Resource locator to the page
        attr --- the name of the attribute
        pattern --- the regex pattern to be applied

        returns True if the pattern does appear
        in the value of the attribute of the input tags
        returns False if it does not
    """
    def ininput(self, url, attr, pattern):
        return
    """the same as above but applied to the <form> tag
    """
    def inform(self, url, attr, pattern):
        return
    """the same as above but applied to the <img> tag

    *PROTIP: this will help you find sites that have been XSSed
    """
    def inimg(self, url, attr, pattern):
        return
    ...
    *PROTIP: this will help you find sites that have been XSSed
    """
    def inscript_tag(self, url, pattern):
        return
    ...
    def inanchor_(self, html, pattern):
        ...
        anchors = soup(html).findAll("a")
        for anchor in anchors:
            if re.search(pattern, str(anchor.get("href"))) is not None:
                print "Found :", anchor
                return True
        return False
