...
class netlib:
    def __init__(self, UA):
        ...
        self.UserAgent = UA
        self.timeout = -1
        self.googleResLimit = 500
        # limiting maximum amount of results we expect from Google
    ...
    def getPage(self, url):
        ...
        try:
            ...
            return (False, 'Socket Error: Could not create connection to '
                           + url)
        try:
            sys.stderr.write("\n\n%s\n%s\n" % (code.replace("\r", ""), msg))
        except:
    ...
    def googleSearch(self, start, query):
        ...
        if query == '':
            return (True, '')
        # change this back to www.google.com
        self.httplib_inst = httplib.HTTP('www.google.com')
        self.httplib_inst.putrequest('GET', '/search?num=' +
                                     str(self.googleResLimit) +
