...
class Result:
    """
        ...
        HTML<type 'str'>  --- one massive string saving all the HTML serverd
        from this server
        headers <type 'list'> --- the list of headers this server responded with

        I've     suggested types for every argument, but the types are
        interchangable (where 'list' is specified)!
    ...
    def __init__(self, URL, anchor_tags, title_tag, texts, HTML):
        self.url = URL
        self.anchor_tags = anchor_tags
        self.title_tag = title_tag
        self.texts = texts
        self.spaces = 5
        # i suspect that i may not need to do this 0_o
        self.mega_list = [self.anchor_tags, self.title_tag, self.texts]
    ...
    """Retuns a printable string of the whatever the result object is
        param --- an attribute of the parameter, this will either be a string,
         or a list of strings

        *I do thought expect to start working with generators instead of lists
         for this method very soon
    ...
    def addAnchor(self, anchor):
        # I likely wont use this, but its good to be prepared
        self.anchor_tags.append(anchor)
    ...
    def _printable(self, param):
        print_string = ""
        if isinstance(param, list) or isinstance(param, generator):
            # just a quick hack to make it more flexible
            print_string = ''.join([">" + " " * self.spaces +
                                   self.cut_string(res)
                                   + "\n" for res in param])
        else:
    ...
    # i was hopping this would be picked up by the print command but its not :/
    def __print__(self):
...
if __name__ == "__main__":
    try:
        # interesting...need to read more about how python calls
        # objects, could maybe hack something to overload an object
        p = Result(["anchor1", "anchor2", "anchor3"],
                   "titlasdfasdfasdfasdfasdfadsfasdfe", ["a", "b", "c"])
    except TypeError:
        p = Result("http://wewon.com", ["anchor1", "anchor2", "anchor3"],
                   "titlasdfasdfasdfasdfasdfadsfasdfe", ["a", "b", "c"])
    p.__print__()
